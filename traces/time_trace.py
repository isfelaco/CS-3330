...
def count_time_in(args, fh):
    ...
    for instruction in csv_reader:
        ...
        if last_instruction != None:
            ...
            ##### item 5 modification #####
            # if forward_from_last: load_use_delay += 1
        ...
        if instruction['is_conditional_branch'] == 'Y':
            if Dict.get(instruction['orig_pc']) == 'Y': jump_taken = True;
            if jump_taken and instruction['branch_taken'] == 'N':
                branch_delay += args.branch_delay
            elif not jump_taken and instruction['branch_taken'] == 'Y':
                branch_delay += args.branch_delay
            Dict[instruction['orig_pc']] = instruction['branch_taken']
